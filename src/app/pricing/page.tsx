    ...
    <>
      ...
      <section className="py-24 md:py-32">
        <div className="container-x grid gap-12 lg:grid-cols-[1.3fr_1fr] lg:items-end">
          ...
          <Reveal delay={0.2} className="border border-line bg-bg-alt p-8">
              ...
              <ul className="space-y-3">
                {shirtStandards.map((s) => (
                  <li key={s} className="flex items-center gap-3">
                    <Check className="size-4 text-accent" strokeWidth={2} /> {s}
                  </li>
                ))}
              </ul>
            </Reveal>
            ...
            {shirtTiers.map((s) => (
              <RevealItem key={s.name}>
                <Tilt max={5} className="h-full">
                  <article className="group flex h-full flex-col overflow-hidden border border-line-soft bg-elevated">
        ...
        <div className="container-x">
          ...
          <RevealGroup className="mt-14 grid gap-5 md:grid-cols-2" stagger={0.1}>
            {suitStandards.map((s, i) => {
              ...
              return (
                <RevealItem key={s.title} className="flex gap-6 border border-accent/30 bg-accent-soft p-8">
                  <Icon className="size-12 shrink-0 text-accent" />
                  <div>
            ...
            {suitCollections.map((c, ci) => (
              <div key={c.title} className="grid gap-12 lg:grid-cols-[1fr_1.6fr] lg:gap-16">
                <div className="lg:sticky lg:top-28 lg:self-start">
                  <MaskReveal className="relative aspect-[4/3]">
                    <Image src={collectionImages[ci]} alt="" fill sizes="(min-width: 1024px) 35vw, 100vw" className="object-cover" />
                  </MaskReveal>
                  <Reveal>
                    <p className="mt-8 font-display text-sm tracking-[0.2em] text-accent">{String(ci + 1).padStart(2, "0")}</p>
                  ...
                  {c.items.map((f) => (
                    <RevealItem as="li" key={f.name} className="group border-b border-line py-7">
                      <div className="flex items-start justify-between gap-6">
                        <h4 className="font-serif text-2xl transition-colors group-hover:text-accent md:text-[1.7rem]">{f.name}</h4>
      ...
      {/* World-class mills */}
      <section className="relative isolate overflow-hidden bg-bg-alt py-28 text-fg md:py-36">
        <div className="container-x">
          <div className="grid gap-12 lg:grid-cols-2">
            <div>
              <p className="eyebrow text-accent">World-class Italian &amp; English mills</p>
              <h2 className="mt-5 font-serif text-5xl leading-[1.05] md:text-6xl">
                The <em className="text-accent-italic">ultimate</em> in cloth.
              </h2>
              <p className="mt-6 text-lg leading-relaxed text-muted">{millsIntro}</p>
              <div className="mt-8 flex gap-4 border border-line p-6">
                <JacketIcon className="size-10 shrink-0 text-accent" />
                <p className="text-muted">{millsIncluded}</p>
              </div>
            </div>
            <RevealGroup as="ul" className="self-start border-t border-line" stagger={0.04}>
              {millFabrics.map((m) => (
                <RevealItem as="li" key={m.name} className="flex items-center justify-between gap-6 border-b border-line py-4">
                  <span className="text-fg">{m.name}</span>
                  <span className="shrink-0 font-display tracking-[0.12em]">
                    <span className="text-accent">{"$".repeat(m.tier)}</span>
                    <span className="text-line">{"$".repeat(4 - m.tier)}</span>
                  </span>
                </RevealItem>
              ))}
            </RevealGroup>
        ...
        <div className="mt-20">
          <Marquee items={mills} className="font-serif text-4xl text-muted italic md:text-6xl" speed={50} />
        </div>
      </section>
      ...
      <section id="quote" className="scroll-mt-24 py-28 md:py-36">
        <div className="container-x grid gap-16 lg:grid-cols-[1fr_1.4fr]">
          ...
          <Reveal className="border border-line-soft bg-elevated p-8 md:p-12">
